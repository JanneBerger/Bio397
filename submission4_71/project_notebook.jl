...
# ╔═╡ b92673c9-e3da-41a8-981d-c17860505c38
using Plots
...
# ╔═╡ 6c42da0c-3c32-43c3-adf2-26b913922c14
using Distributions

# ╔═╡ 5a7d366b-d6aa-4997-b1f6-16817e3744fb
using ScikitLearn.CrossValidation: cross_val_score

# ╔═╡ 12aade1d-ceff-4ab2-bc10-98a86eafc2f8
...
# ╔═╡ 2a89375a-3dd7-447a-80a1-0a09a8fe113e
describe(data_test)
...
# ╔═╡ e8c0c10a-495d-46ce-983c-4be7721fc6b1
gr()

# ╔═╡ 5082d912-0db6-4e4e-9cb9-2dfb2e460871
...
# ╔═╡ 39d3880f-4081-4a82-b8f5-c3bde1ba9da0
begin
	gr()
...
# ╔═╡ ff6a304a-acee-480f-9a8c-d32ad666f1e8
clean_function = ingredients("final_function.jl")

# ╔═╡ 6683b815-19ca-471b-aaed-69c3a2a7a455
df_train_X_all, train_y = clean_function.final_function_train(data_train)

# ╔═╡ 134c1a37-a662-4cb7-a043-c20eba0509df
df_train_X =df_train_X_all

# ╔═╡ 65d21c0d-0c8c-47b7-9a62-0b8754fda63e
df_test_X_all = clean_function.final_function_test(data_test)

# ╔═╡ e3403a48-3b7a-4c19-b994-aa7d3ff6f50c
df_test_X =df_test_X_all

# ╔═╡ 619dd1a2-5657-465c-b5fb-ac557bc4f9c2
...
# ╔═╡ faf08f7c-26ee-407a-baf8-9bd6d0bc64d9
md"""
### Gridsearching
"""

# ╔═╡ a22339a2-eff3-4554-8d6c-e9b27fb36ca1
bestfit = ingredients("find_best_fit.jl")

# ╔═╡ f4206e8e-bccd-44ab-b618-b33f7ab35d40
findfit = bestfit.find_best_fit

# ╔═╡ 860802a3-0c1f-43c8-ba27-a17fe64f26bf
findfit(train_X,train_y,LogisticRegression(),Dict("C"=> 0.1:0.1:1.5, "class_weight" => ("balanced", nothing)))
# using default

# ╔═╡ d230e641-505d-4071-86cd-a7e0951bebce
findfit(train_X,train_y,KNeighborsClassifier(),Dict("n_neighbors" => 1:10, "weights" => ("uniform", "distance")))
# using n_neighbors=3

# ╔═╡ e994162b-2d00-4c6a-8706-f708111712f8
begin
	minsplits_dt = []
	for i in 1:5
	push!(minsplits_dt,findfit(train_X,train_y,DecisionTreeClassifier(), Dict("criterion" => ("gini", "entropy", "log_loss"),"class_weight" => ("balanced", nothing), "min_samples_split" => 6:2:20)))
	end
end
# using default

# ╔═╡ cf146626-63e8-41ed-a0a0-61938d7e1c73
minsplits_dt # using min_samples_split=16

# ╔═╡ 917b6a16-3a3d-4eaa-b301-90d5c15d269f
begin
	minsplits = []
	for i in 1:5
		push!(minsplits,findfit(train_X,train_y,RandomForestClassifier(), Dict("min_samples_split" => 6:10)))
	end
end

# ╔═╡ bad04a80-b0d1-41b4-81b7-6a73ad384369
...
# ╔═╡ 16b91ab8-a734-483b-a29f-7bfaa2dd4724
...
cv_kneighbors = cross_val_score(KNeighborsClassifier(n_neighbors=3),train_X,train_y,cv=5)
...
cv_destree = cross_val_score(DecisionTreeClassifier(min_samples_split=16),train_X,train_y,cv=5)
...
cv_randforest = cross_val_score(RandomForestClassifier(min_samples_split=8),train_X,train_y,cv=5)
...
# ╔═╡ 928d1782-08ce-4183-a6bf-9a26a5728cef
...
# ╔═╡ 72d2777d-9965-443f-9151-c677c311fab6
cv_test = cross_val_score(voting_clf_test,train_X,train_y,cv=5)

# ╔═╡ 216f5fff-60f0-427f-b809-a730f5c0642b
print(mean(cv_test))
...
model = fit!(voting_clf_hard,train_X,train_y)

# ╔═╡ 8c0f70cc-c56a-460a-abb2-73e7f864479f
y_pred = predict(model,test_X)

# ╔═╡ 57fb701a-bb22-4a66-ae1b-d1d6cd9e4251


# ╔═╡ bceada08-4871-4b40-a02d-97e674ef1f0f
df_submission = DataFrame("PassengerId" => 892:1309,
                 "Survived" => y_pred)

# ╔═╡ 5df4079d-0b29-4489-a068-d1ab7283892d
submissionfile = "submission.csv"

# ╔═╡ 0e877ccd-36a8-47c5-87f5-5e7919d475ba
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "e07c12573a97b068d40e6561c8260e2aa599de1f"
